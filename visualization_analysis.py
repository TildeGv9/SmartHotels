...
# Dati statici di fallback (usati se non sono disponibili esperimenti salvati)
_FALLBACK_DATA = [
    {'randomness': 0.2, 'dept_accuracy': 0.76, 'sent_accuracy': 0.81, 'dept_f1': 0.76, 'sent_f1': 0.81},
    {'randomness': 0.4, 'dept_accuracy': 0.57, 'sent_accuracy': 0.70, 'dept_f1': 0.57, 'sent_f1': 0.70},
    {'randomness': 0.9, 'dept_accuracy': 0.31, 'sent_accuracy': 0.93, 'dept_f1': 0.31, 'sent_f1': 0.93},
]
...
class SmartHotelsVisualizer:
    ...
    def __init__(self, pipeline_name=None):
        ...
        # Carica dati sperimentali dal JSON più recente (o usa fallback statico)
    ...
    def _load_experiment_data(self):
        """Carica i dati dall'esperimento più recente per la pipeline corrente.

        Cerca il file JSON più recente in experiments/ corrispondente alla pipeline.
        Se non trovato, usa i dati statici di fallback.
        """
        experiments_dir = Path('experiments')
        pattern = (f'randomness_experiment_{self.pipeline_name}_*.json'
                   if self.pipeline_name else 'randomness_experiment_*.json')

        json_files = sorted(experiments_dir.glob(pattern)) if experiments_dir.exists() else []
        ...
        if json_files:
            ...
            data = _FALLBACK_DATA
            print("⚠️  Nessun esperimento trovato, uso dati statici di esempio.")

        self.randomness_values = [r['randomness'] for r in data]
        self.reparto_accuracy  = [r['dept_accuracy'] for r in data]
        self.sentiment_accuracy = [r['sent_accuracy'] for r in data]
        self.dept_f1_scores    = [r['dept_f1'] for r in data]
        self.sent_f1_scores    = [r['sent_f1'] for r in data]
    ...
    def plot_f1_heatmap(self, save=True):
        """Grafico 3: Heatmap F1-Score (macro) per Reparto e Sentiment al variare del RANDOMNESS.

        I dati provengono dal JSON dell'esperimento più recente caricato in __init__.
        Le colonne rappresentano i due task (Reparto, Sentiment);
        le righe rappresentano i valori di RANDOMNESS testati.
        """
        pivot_data = np.array(list(zip(self.dept_f1_scores, self.sent_f1_scores)))

        row_labels = [f'RANDOMNESS = {r}' for r in self.randomness_values]

        fig, ax = plt.subplots(figsize=(8, max(4, len(self.randomness_values) * 0.9 + 2)))
        sns.heatmap(pivot_data,
                    xticklabels=['Reparto (dept F1)', 'Sentiment (sent F1)'],
                    yticklabels=row_labels,
                    annot=True, cmap='RdYlBu_r', center=0.5, vmin=0.0, vmax=1.0,
                    fmt='.2f', cbar_kws={'label': 'F1-Score Macro'},
                    linewidths=0.5, ax=ax)

        ax.set_title('F1-Score Macro al Variare del RANDOMNESS',
                    fontsize=16, fontweight='bold', pad=20)
